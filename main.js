...
const app = {
    ...
    renderHome: function(container) {
        // Filtrar países para la barra de búsqueda
        const searchResults = this.state.searchTerm 
            ? COUNTRIES_LIST.filter(c => c.name.toLowerCase().includes(this.state.searchTerm.toLowerCase()))
            : [];

        // Generar SVG del Mapa
        let mapPathsHtml = '';
        ...
        <div class="animate-in fade-in duration-500">
            <div class="text-center mb-10 max-w-3xl mx-auto">
                ...
                <p class="text-lg text-slate-500 mb-8">
                    Plataforma unificada para el acceso, comparación y análisis de normativas regulatorias en salud para la región.
                </p>
                ...
                <div class="max-w-md mx-auto relative group z-20">
                    ...
                    <div class="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-30">
                        ...
                        `).join('') : '<div class="p-4 text-slate-400 text-sm text-center">No se encontraron resultados</div>'}
                    </div>` : ''}
                </div>
            </div>
        ...
        document.getElementById('home-search-input').addEventListener('input', (e) => {
            ...
            this.renderHome(container); // Re-render for dropdown
            // Focus back input logic would be needed for perfect UX in vanilla re-render, 
            // but for simplicity we assume click navigation.
    ...
    renderCountryDetail: async function(container) {
        ...
        try {
            ...
            const flagCode = summary.pais === 'Estados Unidos' ? 'us' : summary.pais === 'Reino Unido' ? 'gb' : String(Object.values(summary)[0] || 'xx').toLowerCase().slice(0,2);
                            ...
                            <div class="bg-white rounded-xl border border-slate-200 shadow-sm transition-all overflow-hidden page-break p-6">
                                ...
                                ${source ? `
                                    <div class="mt-4 pt-4 border-t border-slate-100">
                                        ...
                                        <div id="source-${qId.replace(/\./g, '-')}" class="hidden mt-3 bg-slate-50 p-3 rounded-lg border border-slate-200 text-xs text-slate-500 font-mono whitespace-pre-wrap animate-in fade-in duration-200">${source.trim()}</div>
                                    </div>` : ''}
                            </div>`;
                        });
                    questionsHtml += `</div>`;
